...
//$PHPTHUMB_CONFIG['cache_maxage'] = 86400 * 100;        // delete cached thumbnails that haven't been accessed in more than [30 days] (value is maximum time since last access in seconds to avoid deletion)
...
$PHPTHUMB_CONFIG['high_security_password']   = '-_YAY!_-';     // required if 'high_security_enabled' is true, must be at least 5 characters long
...
function phpThumbURL($ParameterString) {
	...
	return str_replace(@$PHPTHUMB_CONFIG['document_root'], '', dirname(__FILE__)).DIRECTORY_SEPARATOR.'i.php?'.$ParameterString.'&hash='.md5($ParameterString.@$PHPTHUMB_CONFIG['high_security_password']);
}
